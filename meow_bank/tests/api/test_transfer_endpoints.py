...
def test_get_transfer(test_client, db_session):
    ...
    create_response = test_client.post("/api/transfers/", json=transfer_data)
    transfer_id = create_response.json()["id"]
    response = test_client.get(f"/api/transfers/{transfer_id}")
...
def test_get_transfer_not_found(test_client):
    response = test_client.get("/api/transfers/00000000-0000-0000-0000-000000000000")
